...
提交 `be43db1`、`3cd8d8c` 已实现参数设置主页、分类/类型、配对管理与解除确认、三档亮度、定时刷新预设/自定义间隔、加载取消、配对码失效恢复和手机桥接不可用页面；定时刷新按设备本地持久化并继续通过 BLE 手机网络请求。`ccad04a` 将箭头/对号改为 LVGL 线条绘制，并为中文 16/20/24 px 字体补回退与 `·` 圆点映射，避免缺字方框。图片页实体按键映射仍位于 Figma 指定的顶部位置。host tests 与 ESP-IDF 完整构建通过，app 大小 `0x1ea560`，尚未烧写真机做触摸、实体按键和视觉验收。
